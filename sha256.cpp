...
vector<uint8_t> SHA256::preProcess(vector<uint8_t> input)
{
    ...
    const uint8_t INITIAL_PADDING = 0x80, // A 1 bit followed by 7 0 bits.
    ...
    {
        input.insert(input.end(), 1, EXTENDED_PADDING); // Build uint8_t by uint8_t and keep checking the size.
    }

    for (int i = LONGWORD_BYTES - 1; i >= 0; i--)
    {
        // Add bytes of inputLength to input, MSB first.
        uint8_t b = (inputLength >> i * BYTE_SIZE) & FULL_BYTE;
...
void SHA256::processChunk(vector<uint8_t> chunkContainer)
{
...
word SHA256::bytesToWord(vector<uint8_t> input, unsigned int start)
{
...
vector<word> SHA256::hash(vector<uint8_t> input)
{
    // Hashes the input and returns the result.

    vector<uint8_t> formatted = preProcess(input), // Do preprocessing and set the result in formatted.
        chunkContainer;
    vector<word> result;
...
{
    // Hashes a string using this class's generic hash(vector<uint8_t> input) function.
...
string SHA256::hash(char* input, longword size)
{
    ...
    // hash values.
    // Uses this class's generic hash(vector<uint8_t> input) function.
